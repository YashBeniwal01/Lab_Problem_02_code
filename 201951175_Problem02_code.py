...
Original file is located at
    https://colab.research.google.com/drive/1qDm7gklmSp0XunE73wEy4ykWOO3gn-Yn
"""
...
pop_age_groups = np.array([0.38, 0.33, 0.23, 0.06])#  population distribution by age group
total_pop = 8700000 # Total population of the city 


#  initial number of normal and covid beds
beds_normal_max = 650
beds_covid_max = 0
discount_factor=0.9
#  cost parameters
...
#  range of beds to convert from normal beds to covid beds
beds_convert_range = np.arange(0, beds_normal_max + 1, 50)

#  transition function
def get_transition_prob(state, action):
    # Unpack the state
    pop_age_groups, total_pop, beds_normal, beds_covid = state
    
    # Calculate the number of requests for both beds
    ...
    # Calculate the number of discharges for both beds
    discharges_normal = poisson.rvs(beds_normal)
    discharges_covid = poisson.rvs(beds_covid,)
    
    # Calculate the number of denied requests for both beds
    ...
    new_total_pop = max(min(total_pop + norm.rvs(scale=0.1), 10000000), 0)
    ...
    return ((new_pop_age_groups, new_total_pop, new_beds_normal, new_beds_covid), reward)

# value iteration function to solve the MDP
            ...
            for beds_normal in range(beds_normal_max+1):
                for week in range(4):
                    ...
                    for action in beds_convert_range:
                        next_state, reward = get_transition_prob((pop_age_groups, total_pop, beds_normal, beds_covid_max), action)
